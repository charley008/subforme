...
func (s *Store) ListNodeDB() ([]Node2, error) {
	rows, err := s.DB.Query(`
		SELECT id, node_id, name, address, COALESCE(port, 443),
		       ...
		       COALESCE(flow, ''), COALESCE(server_name, ''), COALESCE(server_id, 0), COALESCE(mihomo_options, '')
	...
	for rows.Next() {
		var n Node2
		if err := rows.Scan(&n.ID, &n.NodeID, &n.Name, &n.Address, &n.Port, &n.Protocol, &n.Network, &n.Flow, &n.ServerName, &n.ServerID, &n.MihomoOptions); err != nil {
			return nil, err
		}
		out = append(out, n)
	}
...
func (s *Store) ReplaceNodes(nodes []Node2) error {
	...
	for _, n := range nodes {
		...
		_, err := tx.Exec(`
			INSERT INTO nodes (node_id, name, address, port, protocol, network, flow, server_name, server_id, mihomo_options)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		`, nid, n.Name, n.Address, n.Port, n.Protocol, n.Network, n.Flow, n.ServerName, n.ServerID, n.MihomoOptions)
		if err != nil {
			return err
		}
	}
